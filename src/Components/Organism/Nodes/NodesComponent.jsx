...
export default function NodesComponent(){ 
  ...
  const { data: childClients, isLoading: childClientLoading } = useGetChildClientsQuery(filters.client_remotik_id, {
    skip: !filters.client_remotik_id, // Skip this query if no client_remotik_id is selected
  });
  ...
  useEffect(() => {
    if (childClients) {
        console.log("childClients - ",childClients);
      ...
      <form onSubmit={handleSubmit} className="flex flex-wrap justify-center gap-4 mb-6">
        {
          isLoadingClients ? (
            ...
            <select name="client_remotik_id" onChange={handleInputChange} className={style}  value={filters.client_remotik_id}>
